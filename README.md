...
6. **放置弹性任务（约束紧的先选位置）**：放置顺序为——有截止时间的任务按截止时间从早到晚最先放；场景受限（仅工作/仅下班后）的任务其次；同级之间才按优先级排序。优先级不决定任务在一天中的先后，只决定窗口稀缺时谁先挑位置、压缩时谁被保护。每个任务在自己的候选块中按精力匹配度和顺序偏好评分选位：可拆分任务优先选能完整容纳的块，其次选拆分后剩余仍不小于最小分段的块，最后才允许留下无法放置的尾巴；不可拆分任务优先找能放下全部时长的块，否则放入能满足最小时长的最大块并截短。
