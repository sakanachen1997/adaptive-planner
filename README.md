...
1. 先扣除普通 Google Calendar 固定事件和不可用时间。
2. 先为所有未完成任务保留最小时长。
3. 如果最小时长总和大于可用时间，才进入计划冲突。
4. 如果最小时长能放下，剩余时间按任务弹性空间、重要性、deadline 和任务类型分配。
5. `minimumMinutes === desiredMinutes` 的任务等价于不可压缩任务。
6. 当天计划会常态显示每个任务的 `想要 / 最小 / 实际` 时长。
